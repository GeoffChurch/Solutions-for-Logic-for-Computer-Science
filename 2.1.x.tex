...
\subsection{2.1.3} \label{subsec:2.1.3}

Given a relation $R$ on a set $A$, prove that $R$ is transitive if and only if $R \circ R$ is a subset of $R$.

\solution[
    By the definition of transitivity, $R$ is transitive if and only if $\forall x,y,z (xRy \land yRz \implies xRz)$. By the definition of composition, $\forall x,y,z (xRy \land yRz \iff x(R \circ R)z)$. Therefore, $R$ is transitive if and only if $\forall x,y (x(R \circ R)y \implies xRy)$, i.e. if and only if $R \circ R$ is a subset of $R$.
]
...
\solution[
    ...
    \begin{itemize}
        ...
        \item
              Transitivity: adopting a point-free notation, showing transitivity amounts to showing $RSRS \subset RS$ (see \nameref{subsec:2.1.3}). Since $RS = SR$, $RSRS = RRSS$. Since $R$ and $S$ are transitive, $RR \subset R$ and $SS \subset S$, and since $M \subset N \implies (\forall x,y (xFMGy \implies xFNGy))$ for any relations $F$ and $G$ (monotonicity), we can conclude $RRSS \subset RS$ and therefore $RSRS \subset RS$.
              \\\\
